...
import 'package:flutter_app/model/available_base.dart';
import 'package:flutter_app/model/db/base.dart';
import 'package:flutter_app/model/db/device.dart';
import 'package:flutter_app/model/db/drug.dart';
import 'package:flutter_app/model/db/patient.dart';
import 'package:flutter_app/model/db/procedure.dart';
...
import 'package:flutter_app/model/available_themes.dart';
import 'package:flutter_app/model/available_currency.dart';
import 'package:flutter_app/model/available_language.dart';
import 'package:flutter_app/model/address.dart';
import 'package:flutter_app/model/state_block.dart';
import 'package:flutter_app/model/vault.dart';
import 'package:flutter_app/model/db/appdb.dart';
import 'package:flutter_app/model/db/account.dart';
import 'package:flutter_app/util/ninja/api.dart';
import 'package:flutter_app/util/ninja/ninja_node.dart';
import 'package:flutter_app/network/model/block_types.dart';
import 'package:flutter_app/network/model/request_item.dart';
import 'package:flutter_app/network/model/request/accounts_balances_request.dart';
import 'package:flutter_app/network/model/request/account_history_request.dart';
import 'package:flutter_app/network/model/request/fcm_update_request.dart';
import 'package:flutter_app/network/model/request/subscribe_request.dart';
import 'package:flutter_app/network/model/request/block_info_request.dart';
import 'package:flutter_app/network/model/request/pending_request.dart';
import 'package:flutter_app/network/model/request/process_request.dart';
import 'package:flutter_app/network/model/response/account_history_response.dart';
import 'package:flutter_app/network/model/response/account_history_response_item.dart';
import 'package:flutter_app/network/model/response/callback_response.dart';
import 'package:flutter_app/network/model/response/error_response.dart';
import 'package:flutter_app/network/model/response/block_info_item.dart';
import 'package:flutter_app/network/model/response/subscribe_response.dart';
import 'package:flutter_app/network/model/response/process_response.dart';
import 'package:flutter_app/network/model/response/pending_response.dart';
import 'package:flutter_app/network/model/response/pending_response_item.dart';
import 'package:flutter_app/util/sharedprefsutil.dart';
import 'package:flutter_app/util/nanoutil.dart';
import 'package:flutter_app/network/account_service.dart';
import 'package:flutter_app/bus/events.dart';
...
class StateContainerState extends State<StateContainer> {
  ...
  AvailableBase curBase = AvailableBase(AvailableBaseEnum.COM);
  ...
  Base selectedBase = Base(
      ...
      selected: true);
  Drug selectedDrug =
      Drug(id: 1, code: 'DR-001', name: 'DESCRIPTION', type: 'type');
  Device selectedDevice =
      Device(id: 1, code: 'DR-001', name: 'DESCRIPTION', type: 'type');
  Patient selectedPatient =
      Patient(id: 1, code: 'DR-001', name: 'DESCRIPTION', age: 0);
  Procedure selectedProcedure =
      Procedure(id: 1, code: 'DR-001', name: 'DESCRIPTION', type: 'type');
  ...
  void _registerBus() {
    ...
    _historyEventSub =
      ...
      if (topItem != null && topItem.fromTransfer) {
        AccountHistoryRequest origRequest =
            sl.get<AccountService>().pop().request;
          ...
          accounts.forEach((account) {
            event.response.balances.forEach((address, balance) {
              ...
              if (address == account.address &&
                ...
                sl
                    .get<DBHelper>()
                    .updateAccountBalance(account, combinedBalance);
              }
            });
          });
        });
  ...
  Future<void> updateWallet({Account account}) async {
    String address =
        await NanoUtil().seedToAddressInIsolate(await getSeed(), account.index);
  ...
  void handleSubscribeResponse(SubscribeResponse response) {
    ...
    if (response.blockCount != null && !wallet.historyLoading) {
      ...
      int count =
          max(max(response.blockCount, 3000), wallet.history.length) + 5;
  ...
  Future<void> handleBlockInfoResponse(BlockInfoItem resp) async {
    ...
    sl.get<AccountService>().queueRequest(
        ProcessRequest(
            block: json.encode(nextBlock.toJson()), subType: nextBlock.subType),
  ...
  void requestPending({String account}) {
    if (wallet.address != null && account == null) {
      sl.get<AccountService>().queueRequest(PendingRequest(
          account: wallet.address,
          count: max(wallet.blockCount ?? 0, 10),
          threshold: receiveThreshold));
      sl.get<AccountService>().processQueue();
    } else {
      sl.get<AccountService>().queueRequest(
          PendingRequest(
              account: account, count: 20, threshold: receiveThreshold),
          fromTransfer: true);
      sl.get<AccountService>().processQueue();
    }
  }
  ...
  Future<void> requestSend(String previous, String destination, String amount,
      ...
      PaymentRequestMessage paymentRequest}) async {
  ...
  Future<void> requestChange(
      String previous, String balance, String representative) async {
    ...
    sl
        .get<AccountService>()
        .queueRequest(BlockInfoRequest(hash: previous), fromTransfer: false);
    sl.get<AccountService>().processQueue();
  }
